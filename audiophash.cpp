...
extern "C" float *readaudio_snd(const char *filename, long *sr, const float nbsecs, unsigned int *buflen) {
...
uint32_t* ph_audiohash(float *buf, int N, int sr, int &nb_frames) {
	...
	std::complex<double> *pF = (std::complex<double>*)malloc(sizeof(std::complex<double>)*nfft);
	...
	const int nb_barks = (int)(floor(nfft_half / 2 + 1));
	double barkwidth = 1.06;

	// double freqs[nb_barks];
	double* freqs = (double*)alloca(nb_barks);
	// double binbarks[nb_barks];
	double* binbarks = (double*)alloca(nb_barks);
