...
void fft_calc(const int N, const double *x, std::complex<double> *X, std::complex<double> *P, const int step, const std::complex<double> *twids) {
	int k;
...
int fft(const double *x, const int N, std::complex<double> *X) {

	std::complex<double> *twiddle_factors = (std::complex<double>*)malloc(sizeof(std::complex<double>)*(N / 2));
	std::complex<double> *Xt = (std::complex<double>*)malloc(sizeof(std::complex<double>)*N);
	int k;
	for (k = 0; k < N / 2; k++) {
		twiddle_factors[k] = polar_to_complex(1.0, 2.0*PI*k / N);
	}
